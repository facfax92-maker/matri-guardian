...
-- Create role enum
CREATE TYPE public.app_role AS ENUM ('fchv', 'supervisor', 'doctor', 'admin');

-- Create profiles table
CREATE TABLE public.profiles (
  id UUID PRIMARY KEY REFERENCES auth.users(id) ON DELETE CASCADE,
  full_name TEXT NOT NULL,
  role app_role NOT NULL DEFAULT 'fchv',
  phone TEXT,
  district TEXT,
  ward TEXT,
...
-- Security definer function to get user's district
...
SET search_path = public
AS $$
  SELECT district FROM public.profiles WHERE id = _user_id
$$;
...
CREATE POLICY "Supervisors can view profiles in their district"
  ON public.profiles FOR SELECT
  USING (
